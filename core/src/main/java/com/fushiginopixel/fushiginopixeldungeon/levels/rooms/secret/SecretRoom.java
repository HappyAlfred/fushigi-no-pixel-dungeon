...
public abstract class SecretRoom extends SpecialRoom {
	...
	private static final ArrayList<Class<? extends SecretRoom>> ALL_SECRETS = new ArrayList<>( Arrays.asList(
			...
			SecretHoardRoom.class, SecretMazeRoom.class, SecretSummoningRoom.class, SecretShopRoom.class));
	...
	private static float[] baseRegionSecrets = new float[]{3.6f, 4.2f, 4.8f, 5.4f, 6.0f};
	...
	public static void initForRun(){
		...
		shop = Random.Int( runSecrets.size() + 1) <= Random.Int( runSecrets.size());
	...
	public static void restoreRoomsFromBundle( Bundle bundle ) {
		...
		if (bundle.contains( ROOMS )) {
			...
			initForRun();
			Fushiginopixeldungeon.reportException(new Exception("secrets array didn't exist!"));
		}
	}
