...
public class CityMidBossLevel extends Level {
	...
	private static final int CENTER	= HALL_LEFT + HALL_WIDTH / 2;
	...
	protected boolean build() {
		...
		//arena
		Painter.fillEllipse( this, HALL_LEFT, TOP + ROOM_HEIGHT + 1, HALL_WIDTH, HALL_HEIGHT, Terrain.EMPTY );

		//exit room
		Painter.fill( this, ROOM_LEFT, TOP, ROOM_WIDTH, ROOM_HEIGHT, Terrain.EMPTY );
		//entrance room
		Painter.fill( this, ROOM_LEFT, TOP + ROOM_HEIGHT + HALL_HEIGHT + 2, ROOM_WIDTH, ROOM_HEIGHT, Terrain.EMPTY );
		
		exit = (TOP + ROOM_HEIGHT / 2 ) * width() + CENTER;
		map[exit] = Terrain.EXIT;
		
		arenaDoor = (TOP + ROOM_HEIGHT + HALL_HEIGHT + 1) * width() + CENTER;
		map[arenaDoor] = Terrain.DOOR;

		exitDoor = (TOP + ROOM_HEIGHT) * width() + CENTER;
		map[exitDoor] = Terrain.LOCKED_DOOR;
		
		entrance = (TOP + ROOM_HEIGHT + HALL_HEIGHT + 4) * width() + CENTER;
	...
	protected void createItems() {
		...
		if (item != null) {
			...
			do {
				pos =
					Random.IntRange( ROOM_LEFT, ROOM_LEFT + ROOM_WIDTH - 1 ) +
					Random.IntRange( TOP + ROOM_HEIGHT + HALL_HEIGHT + 2, TOP + ROOM_HEIGHT + HALL_HEIGHT + ROOM_HEIGHT + 1 ) * width();
			} while (pos == entrance);
	...
	public void press( int cell, Char hero ) {
		...
		if (!enteredArena && outsideEntraceRoom( cell ) && hero == Dungeon.hero) {
			...
			int count = 0;
			boss.pos = (TOP + ROOM_HEIGHT + HALL_HEIGHT / 2 + 1) * width() + CENTER;
