...
import com.fushiginopixel.fushiginopixeldungeon.effects.particles.WebParticle;
		...
		for (int i = area.left; i < area.right; i++){
			for (int j = area.top; j < area.bottom; j++){
				...
				if (off[cell] > 0) {
					...
					if (ch != null && !ch.isImmune(this.getClass(),new EffectType(EffectType.BLOB,0))) {
						Buff.prolong( ch, Roots.class, TICK,new EffectType(EffectType.BLOB,0) );
					}
				}
			}
		}
