...
public class TearGas extends Blob {
		...
		for (int i = area.left; i < area.right; i++) {
			for (int j = area.top; j < area.bottom; j++) {
				...
				if (cur[cell] > 0 && (ch = Actor.findChar(cell)) != null) {
					if (!ch.isImmune(this.getClass(),new EffectType(EffectType.GAS+EffectType.BLOB,EffectType.SPIRIT))) {
						Buff.prolong(ch, Blindness.class, 5,new EffectType(EffectType.GAS+EffectType.BLOB,EffectType.SPIRIT));
	...
	public void use( BlobEmitter emitter ) {
		...
		emitter.pour( Speck.factory( Speck.TEARGAS ), 0.4f );
	}
