		...
		for (int i = area.left-1; i <= area.right; i++) {
			for (int j = area.top-1; j <= area.bottom; j++) {
				...
				if (cur[cell] > 0) {
					...
					if (ch != null && !ch.isImmune(this.getClass(), new EffectType(EffectType.BLOB,EffectType.ICE))) {
						if (ch.buff(Frost.class) != null){
							Buff.affect(ch, Frost.class, 2f, new EffectType(EffectType.BLOB,EffectType.ICE));
						} else {
							Buff.affect(ch, Chill.class, water[cell] ? 5f : 3f, new EffectType(EffectType.BLOB,EffectType.ICE));
							Chill chill = ch.buff(Chill.class);
							if (chill != null && chill.cooldown() >= 10f){
								Buff.affect(ch, Frost.class, 5f, new EffectType(EffectType.BLOB,EffectType.ICE));
							}
						}
					}
	...
	public static boolean affect( int cell, Fire fire ) {
		...
		if (ch != null) {
			if (Dungeon.level.water[ch.pos]){
				Buff.prolong(ch, Frost.class, Frost.duration(ch) * Random.Float(5f, 7.5f),new EffectType(EffectType.BLOB,EffectType.ICE));
			} else {
				Buff.prolong(ch, Frost.class, Frost.duration(ch) * Random.Float(1.0f, 1.5f),new EffectType(EffectType.BLOB,EffectType.ICE));
			}
		}
