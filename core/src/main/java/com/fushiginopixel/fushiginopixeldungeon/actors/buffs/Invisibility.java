...
public class Invisibility extends FlavourBuff {
	...
	public boolean attachTo( Char target, EffectType type ) {
		if (super.attachTo( target,type )) {
			...
			if (target instanceof Hero && ((Hero) target).subClass == HeroSubClass.ASSASSIN){
				Buff.affect(target, Preparation.class, new EffectType(0,EffectType.SPIRIT));
			}
			return true;
		} else {
			return false;
	...
	public static void dispel(Char c) {
		...
		CloakOfShadows.cloakStealth cloakBuff = c.buff( CloakOfShadows.cloakStealth.class );
		if (cloakBuff != null) {
			cloakBuff.dispel();
		}
		//this isn't a form of invisibilty, but it is meant to dispel at the same time as it.
		TimekeepersHourglass.timeFreeze timeFreeze = c.buff( TimekeepersHourglass.timeFreeze.class );
		if (timeFreeze != null) {
			timeFreeze.detach();
		}
	}
