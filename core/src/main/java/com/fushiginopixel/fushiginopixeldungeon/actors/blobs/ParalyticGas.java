		...
		for (int i = area.left; i < area.right; i++) {
			for (int j = area.top; j < area.bottom; j++) {
				...
				if (cur[cell] > 0 && (ch = Actor.findChar(cell)) != null) {
						if (!ch.isImmune(this.getClass(),new EffectType(EffectType.BLOB + EffectType.GAS,EffectType.POISON)))
						Buff.prolong(ch, Paralysis.class, Paralysis.DURATION,new EffectType(EffectType.BLOB + EffectType.GAS,EffectType.POISON));
				}
			}
		}
	}
