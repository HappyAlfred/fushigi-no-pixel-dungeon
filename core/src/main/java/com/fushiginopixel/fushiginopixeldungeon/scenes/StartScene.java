...
import com.fushiginopixel.fushiginopixeldungeon.windows.WndGameInProgress;
	...
	private static class SaveSlotButton extends Button {
		...
		public void set( int slot ){
			...
			if (newGame){
				...
				if (hero == null){
					hero = new Image(info.heroClass.spritesheet(), 0, 0, 12, 15);
					add(hero);
					...
					hero.copy(new Image(info.heroClass.spritesheet(), 0, 0, 12, 15));
		...
		protected void onClick() {
			if (newGame) {
				Fushiginopixeldungeon.scene().add( new WndStartGame(slot));
			} else {
				Fushiginopixeldungeon.scene().add( new WndGameInProgress(slot));
			}
		}
	}
}
