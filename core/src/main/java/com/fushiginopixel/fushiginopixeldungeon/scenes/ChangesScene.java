...
import com.fushiginopixel.fushiginopixeldungeon.effects.BadgeBanner;
...
public class ChangesScene extends PixelScene {
	...
	public void create() {
		...
		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.GRADIUS, null), "新物品",
				"_-_新增了一种武器！\n"+
						"_-_新增一种稀有的食物——凝胶！用1个凝胶、2种不同的药剂各1瓶可以调和为2瓶爆炸药剂！\n"));

		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.GOLDSWORD, null), "新特性",
				"_-_随着腐蚀类怪物的出现，一些装备的特征得以利用，那就是镀金！\n"+
		...
		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.CANNONBALL, null), "炮弹",
				"_-_炮弹被玩家使用“大炮之壶”发射时，会基于楼层进行一些伤害修正。\n"+
						"_-_炮弹的伤害能够受神射之戒的加成了。"));

		//**********************
		//       v1.2.1
		//**********************
		changes = new ChangeInfo("v1.2.1", true, "");
		changes.hardlight( Window.TITLE_COLOR );
		infos.add(changes);
		...
		changes.addButton( new ChangeButton(new Image(Assets.NPCLYNN,  0, 0, 12, 16), "生物调整",
				"地牢0层的npc增加"));
		...
		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.SOMETHING, null), "新系统",
				"物品现在带有一个标记名。\n"+
		...
		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.POT_NORMAL, null), "外观调整",
				"现在壶的内部与仓库里的物品会显示贴图了"));
		...
		//       v1.2
		//**********************
		changes = new ChangeInfo("v1.2", true, "");
		changes.hardlight( Window.TITLE_COLOR );
		infos.add(changes);
		...
		changes.addButton( new ChangeButton(new Image(Assets.NPCROBERRY,  0, 0, 16, 16), "生物调整",
				"怪物现在有一种独特的“混乱”ai，在怪物被眩晕时会执行\n"+
						"地牢0层的npc增加\n"));

		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.WAND_LIVING_EARTH, null), "新增法杖",
				"新增了2种法杖\n"+
		...
		changes.addButton( new ChangeButton(new Image(Assets.MAGE, 0, 0, 12, 15), "法师",
				"魔弹法杖的战法灌注效果改为攻击命中时有概率射出一发魔弹\n"+
						"术士可以缓慢地回复法杖充能。"));

		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.SOMETHING, null), "系统调整",
				"伤害、buff、环境带有“效果种类”的属性\n"+
						"_-_效果种类由施加种类、表现种类和来源构成。其中施加种类分为近程、远程、爆炸、环境、气体、内部、魔弹、射线、buff。表现种类分为冰、火、电、毒、酸、光、暗、精神。\n"+
						"_-_部分怪物的抗性调整。"));

		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.MAGES_STAFF, null), "法杖调整",
				"法杖不能自动回复能量，电荷也无法给法杖回复能量了。能量戒指的效果被废掉了，以后要改造一下。\n"+
		...
		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.WAR_HAMMER, null),"战锤",
				"战锤现在带有“击晕”特性"));

		//**********************
		//       v1.1
		//**********************
		changes = new ChangeInfo("v1.1", true, "");
		changes.hardlight( Window.TITLE_COLOR );
		infos.add(changes);
		...
		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.SCROLL_OF_ESCAPE, null), "新增卷轴",
				"新增了5种常规卷轴和1种特殊卷轴\n"+
		...
		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.SCROLL_BERKANAN, null),"卷轴调整",
				"卷轴相关的改动\n"+
		...
		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.SCROLL_CERSCEN, null), "以物品为目标的卷轴",
				"以物品为目标的卷轴改动\n"+
						"_-_在得到鉴定之前，读这类卷轴不会提示选择哪一类物品，如果选择了错误的物品就会被浪费了\n"+
						"_-_读了这类卷轴后，无论是否对物品生效，都会自动鉴定出这是什么卷轴\n"));

		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.ONIGIRI, null),"饥饿调整",
				"饥饿相关的改动\n"+
						"_-_玩家的饥饿上限增加一倍，因极度饥饿而受到的伤害变成1%的最大生命/回合\n"+
						"_-_饭团不再每层都会出现了，但是会随机出现在楼层的补给中，并且有概率会出现大饭团\n"));

		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.POTION_AMBER, null), "药品调整",
				"药品相关的改动\n"+
		...
		//       v1.0
		//**********************
		changes = new ChangeInfo("v1.0", true, "");
		changes.hardlight( Window.TITLE_COLOR );
		infos.add(changes);
		...
		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.WORN_SHORTSWORD, new Wayward().glowing()), "Wayward curse",
	...
	private static class ChangeButton extends Component {
		...
		protected void onClick() {
			Fushiginopixeldungeon.scene().add(new ChangesWindow(new Image(icon), title, message));
		}
