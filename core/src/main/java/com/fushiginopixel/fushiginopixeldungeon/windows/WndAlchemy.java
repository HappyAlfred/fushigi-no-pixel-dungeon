...
public class WndAlchemy extends Window {
	...
	public WndAlchemy(){
			...
			for (int i = 0; i < inputs.length; i++) {
				inputs[i] = new WndBlacksmith.ItemButton() {
					...
					protected void onClick() {
						...
						GameScene.selectItem(itemSelector, WndBag.Mode.ALL, Messages.get(WndAlchemy.class, "select"));
					}
				};
		...
		RedButton btnRecipes = new RedButton(Messages.get(this, "recipes_title")){
			...
			protected void onClick() {
				super.onClick();
				Fushiginopixeldungeon.scene().addToFront(new WndMessage(Messages.get(WndAlchemy.class, "recipes_text")));
			}
		};
				...
				if (item != null && inputs[0] != null) {
					for (int i = 0; i < inputs.length; i++) {
						if (inputs[i].item == null) {
							...
							GameScene.show( new WndOptions( "", Messages.get(WndAlchemy.class, "add_in"),
									Messages.get(WndAlchemy.class, "one"), Messages.get(WndAlchemy.class, "all") ) {
								@Override
								protected void onSelect( int index ) {
									if (inputItem.isEquipped( Dungeon.hero )) {
										if(!((EquipableItem)inputItem).doUnequip( Dungeon.hero, false )) return;
										inputs[a].item(inputItem);
										updateState();
									}
									else {
										switch (index) {
											case 0:
												inputs[a].item(inputItem.detach(Dungeon.hero.belongings.backpack));
												updateState();
												break;
											case 1:
												inputs[a].item(inputItem.detachAll(Dungeon.hero.belongings.backpack));
												updateState();
												break;
										}
									}
	...
	private void combine(){
		...
		if (result != null){
			...
			synchronized (inputs) {
				for (int i = 0; i < inputs.length; i++) {
					if (inputs[i] != null && inputs[i].item != null) {
						if (inputs[i].item.quantity() <= 0 || (!inputs[i].item.stackable && inputs[i].item == result)) {
			...
			ingredients = filterInput(Item.class);
