...
public class WndTradeItem extends Window {
	...
	public WndTradeItem( final Heap heap, boolean canBuy ) {
		...
		if (canBuy) {
			...
			RedButton btnRob = new RedButton( Messages.get(this, "rob") ) {
				@Override
				protected void onClick() {
					GameScene.show(

					new WndOptions( Messages.get(WndTradeItem.class, "rob"),
							Messages.get(WndTradeItem.class, "rob_alter"),
							Messages.get(WndTradeItem.class, "rob_yes"),
							Messages.get(WndTradeItem.class, "rob_no") ) {
						@Override
						protected void onSelect( int index ) {
							if (index == 0) {
								if (Dungeon.level instanceof RegularLevel && (((RegularLevel)Dungeon.level).room(heap.pos) != null)){
									Room r = ((RegularLevel) Dungeon.level).room(heap.pos);
									int cell;
									...
									for (Mob mob : Dungeon.level.mobs) {
										if (mob instanceof Shopkeeper) {
											mob.yell(Messages.get(mob, "robber"));
											((Shopkeeper) mob).flee();
											break;
										}
									}
			...
			btnRob.setRect(0, btnBuy.bottom() + GAP, WIDTH, BTN_HEIGHT);
			add( btnRob );
			...
			if (thievery != null) {
				...
				btnSteal.setRect(0, btnRob.bottom() + GAP, WIDTH, BTN_HEIGHT);
				add(btnSteal);

				btnCancel.setRect( 0, btnSteal.bottom() + GAP, WIDTH, BTN_HEIGHT );
			} else
				btnCancel.setRect( 0, btnRob.bottom() + GAP, WIDTH, BTN_HEIGHT );
