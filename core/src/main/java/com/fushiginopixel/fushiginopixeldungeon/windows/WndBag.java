	...
	private class ItemButton extends ItemSlot {
		...
		public void item( Item item ) {
			...
			if (item != null) {
				...
				if (item.name() == null) {
					...
					enable(
						mode == Mode.FOR_SALE && !item.isUnique() && (item.price() > 0) && (!item.isEquipped( Dungeon.hero ) || !item.cursed) ||
						...
						mode == Mode.POT && (item instanceof Pot) ||
		...
		protected boolean onLongClick() {
			if (listener == null && item.defaultAction != null) {
				hide();
				Dungeon.quickslot.setSlot( 0 , item , item.defaultAction);
