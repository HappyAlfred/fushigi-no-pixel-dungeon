...
import com.fushiginopixel.fushiginopixeldungeon.scenes.IntroScene;
...
public class WndStartGame extends Window {
	...
	public WndStartGame(final int slot){
		...
		float heroBtnSpacing = (WIDTH - HeroClass.values().length*HeroBtn.WIDTH)/(float)(HeroClass.values().length);
	...
	private static class HeroBtn extends Button {
		...
		HeroBtn ( HeroClass cl ){
			...
			if (cl == HeroClass.WARRIOR){
				hero = new Image(Assets.WARRIOR, 0, 0, 12, 15);
			} else if (cl == HeroClass.MAGE){
				hero = new Image(Assets.MAGE, 0, 0, 12, 15);
			} else if (cl == HeroClass.ROGUE){
				hero = new Image(Assets.ROGUE, 0, 0, 12, 15);
			} else if (cl == HeroClass.HUNTRESS){
				hero = new Image(Assets.HUNTRESS, 0, 0, 12, 15);
			} else if (cl == HeroClass.FUURAI){
				hero = new Image(Assets.FUURAI, 0, 0, 12, 15);
			}
			add(hero);
		...
		public void update() {
			...
			if (cl != GamesInProgress.selectedClass){
				if ((cl == HeroClass.HUNTRESS && !Badges.isUnlocked(Badges.Badge.BOSS_SLAIN_1)) ||
						(cl == HeroClass.FUURAI && !Badges.isUnlocked(Badges.Badge.HAPPY_END))
						){
					hero.brightness( 0f );
				} else {
		...
		protected void onClick() {
			...
			}else if(cl == HeroClass.FUURAI && !Badges.isUnlocked(Badges.Badge.HAPPY_END)){
				Fushiginopixeldungeon.scene().add(
						new WndMessage(Messages.get(WndStartGame.class, "fuurai_unlock")));
			} else{
	...
	private class HeroPane extends Component {
		...
		protected void createChildren() {
			...
			heroItem = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					Fushiginopixeldungeon.scene().add(new WndMessage(Messages.get(cl, cl.name() + "_desc_item")));
				}
			};
			...
			heroLoadout = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					Fushiginopixeldungeon.scene().add(new WndMessage(Messages.get(cl, cl.name() + "_desc_loadout")));
				}
			};
			...
			heroMisc = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					Fushiginopixeldungeon.scene().add(new WndMessage(Messages.get(cl, cl.name() + "_desc_misc")));
				}
			};
			...
			heroSubclass = new IconButton(new ItemSprite(ItemSpriteSheet.MASTERY, null)){
				...
				protected void onClick() {
					...
					Fushiginopixeldungeon.scene().add(new WndMessage(msg));
				}
			};
				...
				if (cl != null) {
					...
					switch(cl){
						case WARRIOR:
							...
							heroMisc.icon(new ItemSprite(ItemSpriteSheet.ONIGIRI, null));
							break;
						case MAGE:
						...
						case HUNTRESS:
							...
							break;
						case FUURAI:
							heroItem.icon(new ItemSprite(ItemSpriteSheet.ONIGIRI, null));
							heroLoadout.icon(new ItemSprite(ItemSpriteSheet.ONIGIRI, null));
							heroMisc.icon(new ItemSprite(ItemSpriteSheet.ONIGIRI, null));
							break;
					}
