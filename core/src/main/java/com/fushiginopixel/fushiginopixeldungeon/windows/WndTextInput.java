...
public class WndTextInput extends Window {
	...
	public WndTextInput( String title, String initialValue, boolean multiLine, String posTxt, String negTxt, Listener listener){
		this( title, initialValue, multiLine ? MAX_LEN_MULTI : MAX_LEN_SINGLE, multiLine, posTxt, negTxt, listener);
	}

	public WndTextInput(final String title, final String initialValue, final int maxLength,
	                    final boolean multiLine, final String posTxt, final String negTxt, Listener listener){
		super();
		...
		Fushiginopixeldungeon.instance.runOnUiThread(new Runnable() {
			@Override
			public void run() {
				...
				textInput = new EditText(Fushiginopixeldungeon.instance);
				...
				Fushiginopixeldungeon.instance.addContentView(textInput, layout);
			}
		});
	}
		...
		if (textInput != null){
			Fushiginopixeldungeon.instance.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					InputMethodManager imm = (InputMethodManager)Fushiginopixeldungeon
					...
					Fushiginopixeldungeon.updateSystemUI();
