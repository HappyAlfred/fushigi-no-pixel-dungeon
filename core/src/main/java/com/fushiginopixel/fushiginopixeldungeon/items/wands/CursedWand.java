	...
	private static void uncommonEffect(final Wand wand, final Char user, final Ballistica bolt){
		switch(Random.Int(4)){
						...
						public void call() {
							...
							switch (Random.Int(2)) {
								case 0:
									...
									target.damage(damage, wand, new EffectType(EffectType.MAGICAL_BOLT,EffectType.DARK));
									target.sprite.emitter().start(ShadowParticle.UP, 0.05f, 10);
									break;
								case 1:
									user.damage( damage, this, new EffectType(EffectType.MAGICAL_BOLT,EffectType.DARK) );
			...
			case 3:
				...
				Buff.prolong(user, Recharging.class, 20f,new EffectType(EffectType.BLOB,EffectType.ELETRIC));
