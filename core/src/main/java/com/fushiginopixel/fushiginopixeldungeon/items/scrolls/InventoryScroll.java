...
public abstract class InventoryScroll extends Scroll {
	...
	public void doRead() {
		...
		if (!isKnown()) {
			...
			GameScene.selectItem( itemSelector, WndBag.Mode.ALL, Messages.get(InventoryScroll.class, "inv_title") );
		} else {
	...
	private void confirmCancelation() {
		GameScene.show( new WndOptions( Messages.titleCase(name()), Messages.get(this, "warning"),
			...
			protected void onSelect( int index ) {
				switch (index) {
				case 0:
					((InventoryScroll)curItem).knownByUse();
					GLog.i(Messages.get(InventoryScroll.this, "nothing"));
