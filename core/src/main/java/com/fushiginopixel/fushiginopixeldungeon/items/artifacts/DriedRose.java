...
import com.fushiginopixel.fushiginopixeldungeon.messages.Languages;
...
public class DriedRose extends Artifact {
	...
	public static class GhostHero extends NPC {
		...
		public void sayBossBeaten(){
			yell( Random.element( VOICE_BOSSBEATEN[ Dungeon.depth==50 ? 1 : 0 ] ) );
			Sample.INSTANCE.play( Assets.SND_GHOST );
		}
		...
		protected boolean act() {
			...
			if (rose == null || (owner == null || !rose.isEquipped(owner))){
				...
				if(delay <= 0) delay = 1f;
				partialRecover+=HT/delay;
				if(partialRecover >=1){
					HP +=(int)partialRecover;
					partialRecover -= (int)partialRecover;
		...
		public int defenseSkill(Char enemy) {
			...
			if (defense != 0 && rose != null && rose.armor != null ){
				defense = Math.round(rose.armor.evasionFactor( this, enemy, defense ));
			}
					...
					if (rose.weapon != null){
						...
						GameScene.selectItem(new WndBag.Listener() {
							...
							public void onSelect(Item item) {
								...
								} else if (item.isUnique() || item instanceof Boomerang) {
						...
						GameScene.selectItem(new WndBag.Listener() {
							...
							public void onSelect(Item item) {
								if (!(item instanceof Armor)) {
									//do nothing, should only happen when window is cancelled
								} else if (item.isUnique() || ((Armor) item).checkSeal() != null) {
									GLog.w( Messages.get(WndGhostHero.class, "cant_unique"));
									hide();
								} else if (!item.isIdentified()) {
									GLog.w( Messages.get(WndGhostHero.class, "cant_unidentified"));
