	...
	public void doRead() {
		...
		for (Mob mob : Dungeon.level.mobs.toArray( new Mob[0] )) {
			if (Dungeon.level.adjacent(mob.pos , curUser.pos)) {
				Buff.affect( mob, Charm.class, 15,new EffectType(0,EffectType.SPIRIT) ).object = curUser.id();
				curUser.sprite.centerEmitter().start( Speck.factory( Speck.HEART ), 0.2f, 5 );

				if (mob.buff(Charm.class) != null){
					count++;
					affected = mob;
				}
			}
	...
	public void empoweredRead() {
		...
		for (Mob mob : Dungeon.level.mobs.toArray( new Mob[0] )) {
			if (Dungeon.level.heroFOV[mob.pos]) {
				...
				if (t != null){
					Buff.prolong(mob, Terror.class, Terror.DURATION*1.5f,new EffectType(0,EffectType.SPIRIT));
					Buff.affect(mob, Paralysis.class, Terror.DURATION*.5f,new EffectType(0,EffectType.SPIRIT));
				}
			}
		}
	}
