...
public class WandOfPrismaticLight extends DamageWand {
	...
	protected void onZap(Ballistica beam) {
		...
		if (Dungeon.level.viewDistance < 6 ){
			if (Dungeon.isChallenged(Challenges.DARKNESS)){
				Buff.prolong( curUser, Light.class, 2f + level(),new EffectType(0,EffectType.LIGHT));
			} else {
				Buff.prolong( curUser, Light.class, 10f+level()*5,new EffectType(0,EffectType.LIGHT));
			}
		}
	...
	private void affectTarget(Char ch){
		...
		if (Random.Int(5+level()) >= 3) {
			Buff.prolong(ch, Blindness.class, 2f + (level() * 0.333f),new EffectType(EffectType.BEAM,EffectType.LIGHT));
		...
		if (ch.properties().contains(Char.Property.DEMONIC) || ch.properties().contains(Char.Property.UNDEAD)){
			...
			ch.damage(Math.round(dmg*1.333f), this , new EffectType(EffectType.BEAM,EffectType.LIGHT));
		} else {
			ch.sprite.centerEmitter().burst( RainbowParticle.BURST, 10+level() );

			ch.damage(dmg, this,new EffectType(EffectType.BEAM,EffectType.LIGHT));
		}
	...
	public void onHit(MagesStaff staff, Char attacker, Char defender, int damage, EffectType type) {
		//cripples enemy
		Buff.prolong( defender, Cripple.class, 1f+staff.level(),new EffectType(EffectType.MELEE,EffectType.LIGHT));
	}
