...
import com.fushiginopixel.fushiginopixeldungeon.actors.mobs.Abeys;
...
public class AbeysSprite extends MobSprite {
    ...
    public void link(Char ch) {
        super.link(ch);
        if (((Abeys)ch).index > -1) play(charging);
    }
	...
	public void zap( int cell ) {
		...
		turnTo( ch.pos , cell );
		play( zap );
        int pierceParam = Ballistica.MAGIC_BOLT;
        final Ballistica shot = new Ballistica( ch.pos, cell, pierceParam);
        ...
        MagicBreath.breathInLevel(parent, this, shot.pathPointF.get(dist),
                ...
                new Callback() {
                    public void call() {
                        ((Abeys)ch).onZapComplete(shot);
                    }});
	}

    public void darkZap( Ballistica shot ) {

        if(((Abeys)ch).index > -1){
            idle();
        }

        parent.add(new Beam.DeathRay(center(), shot.collisionPointF));
        ((Abeys)ch).darkBeam(shot, true);
    }
